...
// Sections are numbered 01-09 so the homepage reads as one argument in sequence.
// The hero opens it and the closing CTA lands it, so neither takes an index.
// TheOne sits at 05, right after Forge: Forge governs what enters production,
// TheOne is the governed kernel that turns intent into verified action — the two
// flagships that are platform, not applied business, before Core's commercial layer.
// Construction sits at 07: the platform has been described and proven by 06, so
...
export default async function HomePage({ params }: PageParams) {
  ...
  return (
    <>
      ...
      <TheOneSection locale={locale} index={5} />
      <OneAICoreSection locale={locale} index={6} />
      <ConstructionSection locale={locale} index={7} />
      <DeveloperSection locale={locale} index={8} />
      <SecuritySection locale={locale} index={9} />
